...
type Props = {
	...
	rangeRadius?: number
	rangeHue?: number
	particleGlowMode?: 'dark' | 'light'
	backgroundColor?: string
}

export const VortexBackground = ({ particleGlowMode = 'light', ...props }: Props) => {
	...
	const baseTTL = props.baseTTL || 50
	...
	const renderGlow = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) => {
		ctx.save()
		ctx.filter = particleGlowMode === 'light' ? 'blur(8px) brightness(200%)' : 'blur(8px) brightness(80%)'
		ctx.globalCompositeOperation = particleGlowMode === 'light' ? 'lighter' : 'darken'
		ctx.drawImage(canvas, 0, 0)
		ctx.restore()

		ctx.save()
		ctx.filter = particleGlowMode === 'light' ? 'blur(4px) brightness(200%)' : 'blur(4px) brightness(80%)'
		ctx.globalCompositeOperation = particleGlowMode === 'light' ? 'lighter' : 'darken'
		ctx.drawImage(canvas, 0, 0)
		ctx.restore()
	}
